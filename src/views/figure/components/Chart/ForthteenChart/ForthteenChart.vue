...
import { getForthteenData } from "@/server/api/businessTwo";
...
// const { run, data, err } = useRequest(getForthteenData, {
//   manual: true,
//   onSuccess: (res) => {
...
const selectChartData = () =>{
  ...
  "工单号",
            ...
            "结果"]
  configData.value=[];
        selectSaleData({type:'20'}).then(response => {
          ...
          if(response.data.length>0){
            response.data.forEach((element,index) => {
              dataList.push([element.dataOne,element.dataTwo,element.dataThree,element.name,element.dataFour,element.dataSix,element.dataFive])
            });
          }
          tempData.data=dataList;
...
const submitForm_data = (scope) =>{
  const param ={id:scope.id,name:scope.name,dataOne:scope.dataOne,dataTwo:scope.dataTwo,dataThree:scope.dataThree
    ,dataFour:scope.dataFour,dataFive:scope.dataFive,dataSix:scope.dataSix
  }
...
<template>
  <div class="center-table" @dblclick="updateData()">
    ...
    <el-dialog title="工单列表" v-model="updateFlag" width="1200px" @close="cancel" append-to-body>
