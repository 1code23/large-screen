const getPie3D = (pieData, internalDiameterRatio, distance, alpha, pieHeight, opacity = 1) => {
  ...
  let startValue = 0;
  let endValue = 0;
  let legendData = pieData.map((item) => item.name);
  let legendBfb = [];
  ...
  const option = {
    ...
    legend: {
      ...
      //     }
      //   }
      //   var arr = ["{a|" + name + "}", "{b|" + toThousands(tarValue.toString()) + "}"];
      //   return arr.join("  ");
    ...
    //       a: {
    //         fontSize: 15,
    //         width: window.innerWidth >= 1600 ? 110 : 80,
    //         color: "rgb(255,255,255,0.9)",
    //         textAling: "left",
    //         align: "left",
    //         verticalAlign: "middle",
    ...
    grid3D: {
      ...
      viewControl: {
        ...
        alpha: 40, //角度
        distance: 125,//调整视角到主体的距离，类似调整zoom
        rotateSensitivity: 1, // 设置为0无法旋转
        zoomSensitivity: 0, // 设置为0无法缩放
        panSensitivity: 0, // 设置为0无法平移
        autoRotate: "auto", // 自动旋转
      },
      left: '-20%', // 饼图的位置
      top: '-18',
      // center: ["10%", "30%"],
